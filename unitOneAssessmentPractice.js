...
// Write a function called capitalizeTheAs the capitalizes all of the lowercase "a"s in a 
// string leaving all other characters the same
...
// HINT: Use an object where the keys are the numbers, and the values are how many times they
// appear in the array.
function mode(arr) {
  let obj = {};
  let max = -Infinity;
...
function median(arr) {
  ...
  if (sorted.length % 2 === 0) {
    return (sorted[Math.ceil(sorted.length / 2)] + sorted[Math.floor((sorted.length - 1) / 2)]) / 2;
  } else {
...
// Question Eight:
// Write a function called addAllStudents that takes in an array of Classroom objects 
// (described below) and returns the total number of students
...
function addAllStudents(arr) {
  return arr.reduce((acc, ele) => acc + ele.numberOfStudents, 0);
}
// Uncomment out the next line to test your solution
// runQ8Tests()
...
function fewestStudents(arr) {
  ...
  let min = Infinity;
  arr.forEach(ele => {
    if (ele.numberOfStudents < min) {
      min = ele.numberOfStudents;
      minObj = ele;
    }
  });
  return minObj;
...
// Your function should use a higher-ordered function (e.g map, filter, reduce, every, sort) in 
// its implementation
...
// Write a function called containsOnlyNumbers that returns whether or not an array contains only 
// numbers
// Your function should use a higher-ordered function (e.g map, filter, reduce, every, sort) in 
// its implementation
...
// Write a function called getAllAdults that takes in an array of Person objects and returns 
// an array with only Person objects with an age of at least 18
...
// Write a function called getAllNames that takes in an array of Person objects and returns a 
// string with all of the names joined together with a ","
function getAllNames(arr) {
  return arr.map(ele => ele.name).join(',');
}
// runQ15Tests()

// The code below is used to test your solutions.  Feel free to look over it, but do not change 
// any of it.
