...
let average = (arr5) => arr5.reduce((a, b) => a + b) / arr5.length;
// Uncomment out the next line to test your solution
runQ5Tests()
...
let arrayOfClassrooms= [{teacher: "Mr. Smith", numberOfStudents: 28}, {teacher: "Ms. Lopez", numberOfStudents: 32}, {teacher: "Professor McGonagall", numberOfStudents: 20}]
...
// Sample input:
 let fewestStudents = [{teacher: "Mr. Smith", numberOfStudents: 28}, {teacher: "Ms. Lopez", numberOfStudents: 32}, {teacher: "Professor McGonagall", numberOfStudents: 20}]
 let smallest = fewestStudents.reduce((acc, currentEl) => {
     if (acc < currentEl) {
         return acc;
     } else {
         return currentEl;
...
// Your function should use a higher-ordered function (e.g map, filter, reduce, every, sort) in its implementation
// Your output should be in the same order as the input
const onlyLongStrings = (arr) =>  arr.filter((str) => {
  if(str.length > 3) {
  return true;
  } else {
    return false;
...
//class Person2 {
  ...
  //}
//}
//let shawn = new Person2('Shawn Q', '4')
//let alicia = new Person2('Alicia', '68')
//shawn.name();
//alicia.name();
//runQ14Tests()
